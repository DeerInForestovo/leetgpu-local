...
// === BEGIN SOLUTION ===
__global__ void color_inversion_kernel(unsigned char* image, int num_pixels) {
...
public:
    ...
    void gpu_solution() override {
        ...
        const int GRID = ceil_div(NUM_PIXELS, BLOCK);
        color_inversion_kernel<<<GRID, BLOCK>>>(d_image, NUM_PIXELS);
        // === END SOLUTION ===
    }
